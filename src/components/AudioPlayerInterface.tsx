    ...
    try {
      ...
      if (fileBlob) {
        try {
          ...
          console.log('设置元数据:', { title, artist, album, hasCover: !!coverImage });
          setMetadata({ title, artist, album, coverImage });
          return; // 已解析完成，提前返回
        } catch (e) {
      ...
      console.log('最终设置元数据:', { title, artist, album, hasCover: !!coverImage });
      setMetadata({
        title,
        artist,
        album,
